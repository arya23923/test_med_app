...
const ProfileForm = () => {
  ...
  return (
    <div className="profile-container" style={{marginTop: "10vh", padding: '4%', paddingLeft:'5%', display:'flex', justifyContent:'center', alignItems:'center'}}>
      {editMode ? (
        <form onSubmit={handleSubmit}>
          ...
          <label>
            Phone number
            <input type="number" name="phone" value={updatedDetails.phone} onChange={handleInputChange}/>
          </label>
        ...
        <div className="profile-details">
          <h1 style={{paddingBottom:'10%'}}>Welcome, {userDetails.name}</h1>
          {/* Implement code to display and allow editing of phone and email similar to above */}
          <span style={{display:'flex', flexDirection:'row', padding:'5%', }}>
            <strong style={{paddingRight:'10%'}}>Email:</strong>
            <p>{userDetails.email}</p>
          </span>
          <span style={{display:'flex', flexDirection:'row'}}>
            <strong style={{padding:'5%', paddingBottom:'10%'}}>Phone number:</strong>
            <p style={{padding:'5%'}}>{userDetails.phone}</p>
          </span>
          <button onClick={handleEdit} style={{width:'15vw', marginLeft:'3vw'}}>Edit</button>
        </div>
      )}
    </div>
  );
